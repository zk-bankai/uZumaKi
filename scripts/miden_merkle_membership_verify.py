...
file_path = 'benchmarks/miden/MerkleTree/MerkleMembership/verify_bench.json'  # Path to your JSON file
...
df = pd.DataFrame({
    'Metric': ['Security Level', 'Compressed Proof Size (bytes)', 'Cycles', 'Memory Usage (bytes)', 'Proof Size (bytes)'],
    'Value': [
        result['metrics']['Security Level'],
...
plt.bar(df['Metric'], df['Value'], color=['blue', 'green', 'red', 'purple', 'orange'])
plt.xlabel('Metric')
plt.ylabel('Value')
