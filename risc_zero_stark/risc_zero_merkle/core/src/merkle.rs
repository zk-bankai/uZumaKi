// Code source : https://github.com/thor314/circuit-examples/blob/main/risc0-merkle/core/src/merkle.rs
...
impl Hasher for ShaHasher {
    // NOTE: RISC0 Sha256 trait currently only provides clean ways to hash data in one shot. To
...
impl Algorithm<Node> for ShaHasher {
    fn hash(&mut self) -> Node {
        let hash: &mut Digest = &mut Impl::hash_bytes(&self.data);
        let data_node = *hash;
        Node::from(data_node)
    }
}